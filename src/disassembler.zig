...
pub fn disassemble_instruction(intel4004: *Intel4004, addr: u12) ![]u8 {
    ...
    const byte2 = if (n_bytes == 2) intel4004.pram.bytes[intel4004.program_counter + 1] else null;
    ...
    try out.appendSlice(" ");
    for (args) |arg| {
        try out.appendSlice(try std.fmt.allocPrint(std.heap.page_allocator, "{d}", .{arg}));
        try out.appendSlice(" ");
    }
